...
def main():
    ...
    fc_top = 81e9
    fc_bot = 77e9
    wavelength = 3e8/((fc_top - fc_bot)/2)
    ...
    tx = Transmitter(f=[61e9, 60e9], f_bot, f_top
                     ...
                     channels=channels)                     
    '''
    tx = Transmitter(f=[fc_top, fc_bot],
                     t=[0, 8.5e-6],
                     tx_power=12,
                     prp=40e-6,
                     pulses=50,
    ...
    target_1 = dict(location=(4*np.cos(np.radians(true_theta[0])), 4*np.sin(np.radians(true_theta[0])),
                    0), speed=(0, 0, 0), rcs=10, phase=0)
    target_2 = dict(location=(4*np.cos(np.radians(true_theta[1])), 4*np.sin(np.radians(true_theta[1])),
                    0), speed=(0, 0, 0), rcs=10, phase=0)
